...
func TestMain(m *testing.M) {
	...
	conn, socksCloser := testSetup()
	exitCode := m.Run()
	testTeardown(exitCode, conn, socksCloser)
	os.Exit(exitCode)
}

func testSetup() (*sql.Conn, io.Closer) {
	// run SOCKS server
	socksServer, err := socks.New(&socks.Config{})
	socksListener, err := net.Listen("tcp", "127.0.0.1:1080")
	if err != nil {
		log.Fatal(err)
	}
	...
	return conn, socksListener
}

func testTeardown(exitCode int, conn *sql.Conn, socksCloser io.Closer) {
	ctx := context.Background()
